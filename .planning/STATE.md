...
status: unknown
last_updated: "2026-09-25T20:30:00.000Z"
progress:
  total_phases: 3
  completed_phases: 3
  total_plans: 4
...
**Current focus:** Phase 48 complete (shipped in v1.6.9 prerelease); next: run the deferred 47-04 AppImage self-update UAT (v1.6.8 → v1.6.9) on a real Linux machine

---

## Current Position

Phase: 48 (pinned-dependencies-and-supported-ci) — complete
Plan: 3 of 3 complete (48-01 verified archives; 48-02 every action on Node 24 and SHA-pinned, guard in CI; 48-03 v1.6.9 dispatch release with all eight assets)
...
- Phase 48 added: Pinned Dependencies and Supported CI -- every third-party archive `libs/build_deps_all.sh` downloads verified against a recorded sha256 before use, protobuf off the mutable git tag, and the CI action pins bumped off the deprecated Node.js 20 runtime.
- Phase 48 Plan 01 complete (2026-09-25, commits ffd71a4d/eb316764/2ec4e13e): added `libs/build_deps_fetch.sh`'s `fetch_verified(url, sha256, out)` -- a sourceable, side-effect-free helper hashing every download with whichever of `sha256sum` or `shasum -a 256` the platform provides (commit `7f83eda` corrected the original assumption: Git Bash on `windows-latest` ships `sha256sum` and no `shasum`, the reverse of macOS) and refusing to leave a mismatched file on disk; rewrote all three native-dependency downloads in `libs/build_deps_all.sh` (zxing-cpp 2.0.0, yaml-cpp 0.7.0, protobuf) to route through it against a pin table matching `packaging/flatpak/io.github.Ogstra.Proxor.yml` byte-for-byte (switching zxing-cpp/yaml-cpp from `.zip` to `.tar.gz` so one hash covers both paths), with protobuf now built from `protobuf-all-21.4.tar.gz` instead of `git clone --recurse-submodules -b v21.4`; added `libs/tests/test-dependency-pins.sh`, an offline (`file://`, no network) drift guard proving the three pins match the Flatpak manifest by URL and that no `git clone`/bare `curl` remain, wired into `package-policy-tests` in CI on both `windows-latest` and `ubuntu-22.04` with no OS guard. SUP-01/SUP-02/SUP-03 marked complete. Editing `libs/build_deps_*.sh` invalidates the `DepsCache-*` cache key, so the first CI run after this merges rebuilds all three native deps from scratch on every matrix entry -- expected, not a regression. Local verification covered `bash -n` on all scripts and the full offline test suite (all passing); the real download-and-build path (`build-cpp`) and the drift guard's first real run under Windows Git Bash are unverified locally, since this machine has no Qt6/cmake/ninja toolchain, and await the next CI run. Plans 48-02 (action-pin bumps) and 48-03 (`workflow_dispatch` release proof) remain.
- Phase 48 Plan 02 complete (2026-09-25, commits 7881d15/b80aa3d/544b282/ce86cb9): every workflow action now runs on Node 24 and is pinned to a 40-hex SHA with its version in a comment -- upload-artifact v7.0.1, download-artifact v8.0.1, setup-go v7.0.0, cache v6.1.0, install-qt-action v4.4.1 (the v4.1.1 SHA ran setup-python v5 and its inner action on Node 20; aqt default moves 3.1 to 3.3, Python >=3.14). gha-setup-ninja and msvc-dev-cmd have no Node 24 release at all, so the ninja setup step was removed (ninja 1.13.2 is preinstalled on ubuntu-22.04 and the windows images) and msvc-dev-cmd was replaced by egor-tensin/vs-shell v2.2, a composite PowerShell action with the same `arch` input -- an owner-approved deviation from "only uses: lines change". upload-artifact from v4.4.0 on skips dotfiles by default, which silently dropped `.SRCINFO` from `release-assets-arch` (PKGBUILD still matched, so if-no-files-found never fired); fixed with `include-hidden-files: true` there. Added `.github/tests/test-action-pins.sh` (rejects floating tags, missing version comments and two SHAs for one action; self-tests against bad fixtures first), wired into package-policy-tests with no OS guard. Verified through the check-run annotations API: zero Node deprecation annotations on push run 36178662655 and on release run 36182003970. SUP-04/SUP-05 complete.
- Phase 48 Plan 03 complete (2026-09-25, commit 3e5cedd plus fix ce86cb9): v1.6.9 cut as the proof release. The first dispatch (run 36179486681) failed at publish on the missing `.SRCINFO`; the half-created release and tag were deleted and the re-dispatch (run 36182003970, tag v1.6.9 at ce86cb9) published all eight assets as a prerelease. SHA256SUMS was re-validated against the downloaded published assets (7/7, covering exactly the 7 binaries). `.SRCINFO` presence in `release-recipes` is proven indirectly, through the upload log and the prepare-final-assets gate; the artifact zip itself was not opened. The winget submission on the fork branch moved to 1.6.9 (commit b6e04591b) while microsoft/winget-pkgs#433900 still carries the 1.6.8 title. SUP-06 complete; Phase 48 complete.
