...
void EditSocksHttp::onStart(std::shared_ptr<ProxorGui::ProxyEntity> _ent) {
    this->ent = _ent;
    auto bean = this->ent->SocksHTTPBean();

    if (bean->socks_http_type == ProxorGui_fmt::SocksHttpBean::type_Socks4) {
        ui->version->setCurrentIndex(1);
    } else {
        ui->version->setCurrentIndex(0);
    }
    if (bean->socks_http_type == ProxorGui_fmt::SocksHttpBean::type_HTTP) {
...
bool EditSocksHttp::onEnd() {
    ...
    if (ui->version->isVisible()) {
        if (ui->version->currentIndex() == 1) {
            bean->socks_http_type = ProxorGui_fmt::SocksHttpBean::type_Socks4;
        } else {
            bean->socks_http_type = ProxorGui_fmt::SocksHttpBean::type_Socks5;
        }
    }
