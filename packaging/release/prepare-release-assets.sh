...
asset_names() { find "$input" -type f \( -name '*.zip' -o -name '*.AppImage' -o -name '*.deb' -o -name '*.rpm' -o -name '*.flatpak' \) -exec basename {} \; ; }
...
mkdir -p "$output/aur"; cp "$pkgbuild" "$srcinfo" "$output/aur/"
