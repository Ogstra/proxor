...
#define DECODE_V2RAY_N_1                                                                                                        \
    ...
    bool ShadowSocksBean::TryParseLink(const QString &link) {
        if (SubStrBefore(link, "#").contains("@")) {
            ...
            auto query = GetQuery(url);
            const auto pluginValue = query.queryItemValue("plugin", QUrl::FullyDecoded);
            auto pluginName = SubStrBefore(pluginValue, ";");
            if (pluginName == "simple-obfs") pluginName = "obfs-local";
            SetPluginParts(pluginName, SubStrAfter(pluginValue, ";"));
        } else {
            // v2rayN
            DECODE_V2RAY_N_1
