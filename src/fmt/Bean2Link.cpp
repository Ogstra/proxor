...
namespace ProxorGui_fmt {
    ...
    QString ShadowSocksBean::ToShareLink() {
        ...
        if (!name.isEmpty()) url.setFragment(name);
        QUrlQuery q;
        const auto pluginName = PluginName();
        if (!pluginName.isEmpty()) {
            auto pluginValue = pluginName;
            const auto pluginOptions = PluginOptions();
            if (!pluginOptions.isEmpty()) pluginValue += ";" + pluginOptions;
            q.addQueryItem("plugin", pluginValue);
        }
    ...
    QString VMessBean::ToShareLink() {
        if (ProxorGui::dataStore->old_share_link_format) {
            // v2rayN format
            QJsonObject N{
                {"v", "2"},
                {"ps", name},
            ...
            if (stream->utlsFingerprint.isEmpty()) {
                query.addQueryItem("fp", ProxorGui::dataStore->utlsFingerprint);
            } else {
