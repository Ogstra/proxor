...
namespace ProxorGui {
    ...
    QString TrimIpv6Brackets(QString value) {
    ...
    void ParseDnsServerHostPort(const QString &input, QString &server, int &port) {
        server = input.trimmed();
        port = -1;

        if (server.startsWith('[')) {
            auto closing = server.indexOf(']');
            if (closing > 0) {
                const auto host = server.left(closing + 1);
                const auto suffix = server.mid(closing + 1);
                if (suffix.startsWith(':')) {
                    bool ok = false;
                    const auto parsedPort = suffix.mid(1).toInt(&ok);
                    if (ok) {
                        port = parsedPort;
                        server = host;
                    }
            ...
            const auto colon = server.lastIndexOf(':');
            bool ok = false;
            const auto parsedPort = server.mid(colon + 1).toInt(&ok);
            if (ok) {
                port = parsedPort;
        ...
        server = TrimIpv6Brackets(server);
    }

    QJsonObject BuildTypedDnsServer(const QString &tag, QString address, const QString &detour = {}, const QString &strategy = {}) {
        QJsonObject dnsServer;
        address = address.trimmed();

        if (!tag.isEmpty()) dnsServer["tag"] = tag;
        if (!strategy.isEmpty()) dnsServer["strategy"] = strategy;
        ...
        if (address.startsWith("rcode://", Qt::CaseInsensitive)) {
            dnsServer["address"] = address;
            if (!detour.isEmpty()) dnsServer["detour"] = detour;
            return dnsServer;
        }
        ...
        if (address.contains("://")) {
            ...
            server = TrimIpv6Brackets(url.host(QUrl::FullyDecoded));
            port = url.port(-1);
            path = url.path();
            if (server.isEmpty()) {
                server = TrimIpv6Brackets(url.authority());
            }
        } else {
            ParseDnsServerHostPort(address, server, port);
        }
        ...
        if (scheme == "https" || scheme == "h3") {
            dnsServer["path"] = (path.isEmpty() || path == "/") ? "/dns-query" : path;
        }
        if (tag != "dns-local" && !server.isEmpty() && !IsIpAddress(server)) {
            dnsServer["domain_resolver"] = "dns-local";
        }
        if (!detour.isEmpty()) dnsServer["detour"] = detour;
    ...
    std::shared_ptr<BuildConfigResult> BuildConfig(const std::shared_ptr<ProxyEntity> &ent, bool forTest, bool forExport) {
        ...
        auto customBean = dynamic_cast<ProxorGui_fmt::CustomBean *>(ent->bean.get());
...
#define IP_USER_RULE                                                             \
    ...
    QString BuildChainInternal(int chainId, const QList<std::shared_ptr<ProxyEntity>> &ents,
        ...
        for (const auto &ent: ents) {
            ...
            if (thisExternalStat > 0) {
                ...
                status->result->extRs.emplace_back(std::make_shared<ProxorGui_fmt::ExternalBuildResult>(extR));
    ...
    void BuildConfigSingBox(const std::shared_ptr<BuildConfigStatus> &status) {
        ...
        auto add_rule_route = [&](const QStringList &list, bool isIP, const QString &out, const QString &action = {}) {
        ...
        {
            QJsonObject domainResolver{{"server", "dns-direct"}};
            if (!dataStore->routing->domain_strategy.isEmpty())
                domainResolver["strategy"] = dataStore->routing->domain_strategy;
            routeObj["default_domain_resolver"] = domainResolver;
        }
        if (status->forExport) {
    ...
    QString WriteVPNSingBoxConfig() {
        // tun user rule
        auto match_out = dataStore->vpn_rule_white ? "proxor-socks" : "direct";
        auto no_match_out = dataStore->vpn_rule_white ? "direct" : "proxor-socks";
