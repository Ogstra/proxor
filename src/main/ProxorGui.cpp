...
namespace ProxorGui {
    ...
    namespace {
        QString NativeExecutableName(const QString &baseName) {
#ifdef Q_OS_WIN
            if (baseName.endsWith(".exe", Qt::CaseInsensitive) || baseName.endsWith(".old", Qt::CaseInsensitive)) {
                return baseName;
            }
            return baseName + ".exe";
#else
    ...
    bool Routing::SetToActive(const QString &name) {
        ProxorGui::dataStore->routing = std::make_unique<Routing>();
        ProxorGui::dataStore->routing->load_control_must = true;
        ProxorGui::dataStore->routing->fn = ROUTES_PREFIX + name;
        auto ok = ProxorGui::dataStore->routing->Load();
        if (ok) {
            ProxorGui::dataStore->active_routing = name;
...
#ifdef Q_OS_WIN
        const auto envRoot = qEnvironmentVariable("PROXOR_PACKAGE_ROOT");
        if (!envRoot.isEmpty()) {
            return QDir(envRoot).absolutePath();
        }

        auto appDir = QDir(QCoreApplication::applicationDirPath());
        if (appDir.dirName().compare("runtime", Qt::CaseInsensitive) == 0) {
            auto parentDir = appDir;
            if (parentDir.cdUp()) {
                if (parentDir.dirName().compare("config", Qt::CaseInsensitive) == 0) {
                    auto packageDir = parentDir;
                    if (packageDir.cdUp()) {
    ...
    QString PackageFilePath(const QString &relativePath) {
        return QDir(PackageRootPath()).filePath(relativePath);
    }

    QString PackageExecutablePath(const QString &baseName) {
        return PackageFilePath(NativeExecutableName(baseName));
    }
    ...
    QString FindProxorCoreRealPath() {
        auto fn = PackageExecutablePath("proxor_core");
...
#else
#ifdef Q_OS_LINUX
        admin |= Linux_GetCapString(FindProxorCoreRealPath()).contains("cap_net_admin");
#endif
        admin |= geteuid() == 0;
#endif
