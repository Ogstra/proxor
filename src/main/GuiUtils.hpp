...
#define D_C_LOAD_STRING(a) CACHE.a = ProxorGui::dataStore->a;
#define D_C_SAVE_STRING(a) ProxorGui::dataStore->a = CACHE.a;
...
#define D_LOAD_STRING(a) ui->a->setText(ProxorGui::dataStore->a);
#define D_LOAD_STRING_PLAIN(a) ui->a->setPlainText(ProxorGui::dataStore->a);
#define D_SAVE_STRING(a) ProxorGui::dataStore->a = ui->a->text();
#define D_SAVE_STRING_PLAIN(a) ProxorGui::dataStore->a = ui->a->toPlainText();
...
#define D_LOAD_INT(a)                                  \
    ui->a->setText(Int2String(ProxorGui::dataStore->a)); \
    ui->a->setValidator(QRegExpValidator_Number);
#define D_SAVE_INT(a) ProxorGui::dataStore->a = ui->a->text().toInt();

#define P_LOAD_COMBO_STRING(a) ui->a->setCurrentText(bean->a);
#define P_SAVE_COMBO_STRING(a) bean->a = ui->a->currentText();

#define D_LOAD_COMBO_STRING(a) ui->a->setCurrentText(ProxorGui::dataStore->a);
#define D_SAVE_COMBO_STRING(a) ProxorGui::dataStore->a = ui->a->currentText();

#define P_LOAD_COMBO_INT(a) ui->a->setCurrentIndex(bean->a);
#define P_SAVE_COMBO_INT(a) bean->a = ui->a->currentIndex();

#define D_LOAD_BOOL(a) ui->a->setChecked(ProxorGui::dataStore->a);
#define D_SAVE_BOOL(a) ProxorGui::dataStore->a = ui->a->isChecked();

#define P_LOAD_BOOL(a) ui->a->setChecked(bean->a);
#define P_SAVE_BOOL(a) bean->a = ui->a->isChecked();

#define D_LOAD_INT_ENABLE(i, e)                             \
    if (ProxorGui::dataStore->i > 0) {                        \
        ui->e->setChecked(true);                            \
        ui->i->setText(Int2String(ProxorGui::dataStore->i));  \
    } else {                                                \
...
#define D_SAVE_INT_ENABLE(i, e)                         \
    if (ui->e->isChecked()) {                           \
        ProxorGui::dataStore->i = ui->i->text().toInt();  \
    } else {                                            \
        ProxorGui::dataStore->i = -ui->i->text().toInt(); \
    }
